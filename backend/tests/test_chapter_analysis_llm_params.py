...
class TestChapterAnalysisLlmParams(unittest.TestCase):
    def _request(self):
    ...
    def test_analyze_chapter_keeps_configured_max_tokens(self) -> None:
        ...
        def fake_call_llm_and_record(**kwargs):  # type: ignore[no-untyped-def]
            ...
            return RecordedLlmResult(
                text='{"chapter_summary":"ok","hooks":[],"foreshadows":[],"plot_points":[],"suggestions":[],"overall_notes":""}',
                finish_reason="stop",
                latency_ms=1,
                dropped_params=[],
                run_id="run-1",
    ...
    def test_auto_memory_update_keeps_configured_max_tokens(self) -> None:
        ...
        def fake_call_llm_and_record(**kwargs):  # type: ignore[no-untyped-def]
            ...
            if len(captured) == 1:
                return RecordedLlmResult(
                    text=(
                        '{"chapter_summary":"ok","hooks":[],"foreshadows":[],'
                        '"plot_points":[],"suggestions":[],"overall_notes":""}'
                    ),
                    finish_reason="stop",
                    latency_ms=1,
                    dropped_params=[],
        ...
        with patch.object(chapter_analysis, "SessionLocal", return_value=_FakeDb()):
            with patch.object(
                ...
                with patch.object(
                    ...
                    "_resolve_task_llm_for_call",
                    side_effect=[
                        SimpleNamespace(api_key="sk-test", llm_call=self._prepared_call(max_tokens=12000)),
                        SimpleNamespace(api_key="sk-test", llm_call=self._prepared_call(max_tokens=9000)),
                    ],
                ):
                    with patch.object(chapter_analysis, "ensure_default_chapter_analyze_preset"):
                        with patch.object(chapter_analysis, "_ensure_default_preset_from_resource"):
                            with patch.object(chapter_analysis, "build_chapter_analyze_render_values", return_value={}):
                                with patch.object(
                                    chapter_analysis,
                                    "render_preset_for_task",
        ...
        self.assertEqual(captured[1].params["temperature"], 0.2)

    def test_rewrite_chapter_keeps_configured_max_tokens(self) -> None:
        captured: list[PreparedLlmCall] = []
        ...
        with patch.object(chapter_analysis, "SessionLocal", return_value=_FakeDb()):
            with patch.object(
                ...
                with patch.object(
                    ...
                    with patch.object(chapter_analysis, "ensure_default_chapter_rewrite_preset"):
                        with patch.object(chapter_analysis, "build_chapter_rewrite_render_values", return_value={}):
                            with patch.object(
                                chapter_analysis,
                                "render_preset_for_task",
                                return_value=("system", "user", None, None, None, None, {}),
                                ...
                                with patch.object(chapter_analysis, "call_llm_and_record", side_effect=fake_call_llm_and_record):
                                    chapter_analysis.rewrite_chapter(
                                        request=self._request(),
                                        chapter_id="chapter-1",
                                        body=ChapterRewriteRequest(analysis={"chapter_summary": "ok"}),
                                        user_id="local-user",
                                    )

        self.assertEqual(captured[0].params["max_tokens"], 12000)
        self.assertEqual(captured[0].params["temperature"], 0.35)

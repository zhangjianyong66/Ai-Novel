...
- R1：按钮点击跳转后，当前差异序号应稳定指向点击目标，不能被同一轮程序化滚动中的滚动监听回写成前一个仍部分可见的差异。
...
实现时需要确认一个产品语义：当用户点击“下一个差异”后，即使上一处差异仍部分可见，计数是否必须立即保持在目标差异上，直到用户主动滚动到别的位置？

推荐答案：是。按钮导航是显式命令，显式命令的目标应优先于同一轮滚动产生的自动同步；用户后续手动滚动时再恢复自动同步。

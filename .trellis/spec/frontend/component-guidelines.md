...
- 章节版本移动端对比这类带 sticky 内部导航的阅读视图，模式切换、基准选择等控制项应放在抽屉/面板顶部工具栏；滚动正文区应直接承载 sticky 导航和正文内容，避免在操作栏与 sticky 导航之间插入独立选择卡片导致空隙或叠层。移动端 sticky 导航应使用不透明背景、直角边缘，并让滚动区顶部 `padding` 为 0，否则正文滚过时会从半透明背景、圆角或顶部内边距处透出。
